...
> **Zero knowledge required. Zero dependencies.** 74 steps. 171 tests. One working Lisp interpreter at the end. Each step first explains **what problem to solve**, then writes the code.
...
> 💡 The gist: You write the recipe for spaghetti bolognese. The interpreter is the chef who reads it and actually cooks the dish.
...
> 💡 Think of it this way: Other languages give you a fixed recipe book. Lisp gives you a kitchen — you can rewrite the recipe as you go. That's why they call it "the programmable programming language."
...
```mermaid
flowchart LR
  A["Preparation      <br/>Steps 1-4  "] --> B["Meet the Values  <br/>Steps 5-9  "] --> C["Make It Compute  <br/>Steps 10-13"] --> D["Split into Words <br/>Steps 14-17"] --> cont["☟"]
  style A fill:#f8f9fa,stroke:#ced4da
  style B fill:#e1f5fe,stroke:#4fc3f7
  style C fill:#f8f9fa,stroke:#ced4da
  style D fill:#e1f5fe,stroke:#4fc3f7
  style cont fill:none,stroke:none,font-size:28px
```


```mermaid
flowchart LR
  E["Understand Words <br/>Steps 18-23"] --> F["Naming Things    <br/>Steps 24-27"] --> G["Real Computation <br/>Steps 28-31"] --> H["More Data Types  <br/>Steps 32-33"] --> cont2["☟"]
  style E fill:#e1f5fe,stroke:#4fc3f7
  style F fill:#f8f9fa,stroke:#ced4da
  style G fill:#f8f9fa,stroke:#ced4da
  style H fill:#f8f9fa,stroke:#ced4da
  style cont2 fill:none,stroke:none,font-size:28px
```


```mermaid
flowchart LR
  I["Make Choices     <br/>Steps 34-37"] --> J["Remember the Past<br/>Steps 38-40"] --> K["Go Faster        <br/>Steps 41-43"] --> L["More Magic       <br/>Steps 44-45"] --> cont3["☟"]
  style I fill:#fff3e0,stroke:#ffb74d
  style J fill:#fff3e0,stroke:#ffb74d
  style K fill:#fff3e0,stroke:#ffb74d
  style L fill:#f8f9fa,stroke:#ced4da
  style cont3 fill:none,stroke:none,font-size:28px
```


```mermaid
flowchart LR
  M["Builtin Functions<br/>Step 46    "] --- _end[" "]
  style M fill:#f8f9fa,stroke:#ced4da
  style _end fill:none,stroke:none
...
Full breakdown:
  ...
  (define square             ← Give the function a name, store it in the global environment
    ...
    │  Function body: (* x x) ← multiplication│
    │  Birth environment: global ← snapshot     │
    └──────────────┘
```
...
make-counter itself:
  ...
  │          (set! start (+ start 1))   │
  │          start)                     │
  │  Birth environment: Global 📸       │
  └─────────────────────────────────────┘

  Heads-up: make-counter's body is itself a lambda!
            Calling make-counter returns that inner lambda,
            not a number.
```
...
Global environment now:
  ...
  └─────────────────────────────────┼────┘
                                    │
                                    │  counter-fn's backpack points here
                                    ▼
                         ┌──────────────────┐
                         │ backpack (alive!)│
                         │  start → 0       │
...
```mermaid
graph LR
  A["Source<br/>Steps 1-7"] --> B["Lexer<br/>Steps 13-20"]
  B --> C["Parser<br/>Steps 21-32"]
  C --> D["Eval<br/>Steps 33-52"]
...
│    Web                    │  Toolchain version: 1.96│  ● Library         │
...
**2 Middle: Change the path.** Find the first line **"Location(L):"** and change the last part of the path from `untitled` to `lisp-rust`:

```
  ❌ .../rust-learning/untitled
  ✅ .../rust-learning/lisp-rust
```
...
```bash
cargo new lisp-rust --lib
cd lisp-rust
```

> Both methods produce the exact same result. After Method 2, just use RustRover's **Open** to open the `lisp-rust` folder.
...
> 🏋️ **Exercises**
> 1. (⭐) Change the project name from `lisp-rust` to whatever you like, then run `cargo test`
...
> 💡 Think of it this way: `u8` is 8 switches, each 0 or 1, giving 256 possible values (0-255). `u16` is 16 switches, giving 65,536 values. Same pattern for the rest.
...
> 💡 Think of it this way: An array is a fixed row of cups — you can't add or remove them. A `Vec` is an expandable cup holder — pour more in and it grows. Downside: occasional "rearranging" (reallocation).
...
> 💡 Think of it this way: `#[derive(Clone, Debug, PartialEq)]` is like telling your assistant "handle copying, printing, and comparing for this type." Doing those three by hand takes 30+ lines of boilerplate; the derive macro finishes in a blink. It's a secretary you dictate intent to, not a form you fill out.
...
> 4. (⭐⭐⭐) **Think first**: If you wrote `eval_str("\"hello\" + 42")`, what do you think would happen
...
>       ▼
> ┌─────────────┐
> │ Is it a number? │──yes──→ Extract 42.0 → Return Ok(Number(42.0))
> └─────────────┘
>       │no
>       ▼
> ┌─────────────┐
> │ _ catch-all │──→ Return Err("This type is not supported yet")
> └─────────────┘
> ```
...
```mermaid
graph LR
  Result["Result&lt;T, E&gt;"] --> Ok["Ok(T)<br/>✅ Wrapped value"]
  Result --> Err["Err(E)<br/>❌ Wrapped error"]
  style Result fill:#f8f9fa,stroke:#495057
...
💡 The gist — `&` means "borrow." You get temporary access without taking ownership.
...
💡 Think of it this way — `use` imports things from other files into the current one. `use crate::{LispExp, LispErr}` says "bring these two types into this file." Without `use`, you'd have to type `crate::LispExp` everywhere.
...
```mermaid
graph TD
  List["List(+, 1, (*, 2, 3))"] --> Plus["+"]
  List --> Num1["Number(1)"]
  List --> Inner["List(*, 2, 3)"]
  Inner --> Star["*"]
  Inner --> Num2["Number(2)"]
  Inner --> Num3["Number(3)"]
  style List fill:#fff3cd,stroke:#ffc107
  style Plus fill:#cce5ff,stroke:#0d6efd
  style Star fill:#cce5ff,stroke:#0d6efd
  style Inner fill:#fff3cd,stroke:#ffc107
  style Num1 fill:#d4edda,stroke:#28a745
  style Num2 fill:#d4edda,stroke:#28a745
  style Num3 fill:#d4edda,stroke:#28a745
```
...
sequenceDiagram
  ...
  participant PA as parse_atom

  P->>RS: sees "(",<br/>calls read_seq
...
💡 Think of it this way — `impl` adds methods to a struct. Like adding buttons to a remote control — each method is a new button.
...
💡 Think of it this way — `Option`: a box that either holds a value or is empty.
...
💡 Think of it this way — A function pointer is like a remote control button: press it, the corresponding function runs.
...
classDiagram
  ...
  LispExp --> LispLambda : Lambda(Box)
  LispExp --> LispExp : List(Vec) recursive
```
...
$ cargo test
running 13 tests
test tests::test_if_true ... ok
test tests::test_if_false ... ok
...
...
test tests::test_eval_symbol ... ok
test tests::test_eval_addition ... ok
test tests::test_tokenize_simple ... ok
test tests::test_tokenize_whitespace ... ok
test tests::test_tokenize_parens ... ok
test tests::test_parse_symbol ... ok
test tests::test_if ... ok
...
$ cargo test
running 16 tests
test tests::test_create_lambda ... ok
...
...
💡 Think of it this way — Calling a lambda is like putting on a play:
...
$ cargo test
running 17 tests
test tests::test_call_lambda ... ok
test tests::test_lambda_with_env ... ok
...
...
💡 The gist — `Rc<RefCell<>>`: Why the complexity?
...
```mermaid
graph TD
  Global["Global env<br/>outer = None<br/>x → 10, + → Func(+)"] -->|outer| Local["Local env<br/>y → 20"]
  LookupX["Lookup x:<br/>① Local env → not found<br/>② outer → Global env<br/>③ found x = 10 ✅"]
  LookupY["Lookup y:<br/>① Local env → found y = 20 ✅"]
  style Global fill:#fff3cd,stroke:#ffc107
  style Local fill:#d4edda,stroke:#28a745
  style LookupX fill:#f8f9fa,stroke:#ced4da
  style LookupY fill:#f8f9fa,stroke:#ced4da
```
...
$ cargo test
running 18 tests
test tests::test_nested_env_lookup ... ok
...
...
💡 Think of it this way: A lambda is like someone who leaves home with a photo album of everything in the house. No matter where they go, they can always find what was "back home." That album is the captured environment.
...
$ cargo test
running 18 tests
test tests::test_closure_captures_env ... ok
...
test result: ok. 18 passed; 0 failed
```
...
sequenceDiagram
  ...
  participant ML as (define ...)
  participant C as Closure
  participant E as Eval

  U->>ML: (define make-adder<br/>(lambda (n)<br/>  (lambda (x) (+ x n))))
  ML->>C: Create closure
  Note over C: env → global env<br/>params → [n]<br/>body → (lambda (x) (+ x n))
  U->>E: ((make-adder 5) 3)
  E->>C: Call outer lambda (n=5)
  Note over C: Return inner closure<br/>env → {n:5}<br/>body → (lambda (x) (+ x n))
...
```mermaid
graph TD
  subgraph TCO["Recursion · O(n) stack frames"]
    A["(loop 10000)"] --> B["call loop(10000)"]
    B --> C["call loop(9999)"]
    C --> D["... 10000 frames"]
    D --> E["💥 Stack Overflow!"]
  end
  subgraph Trampoline["Trampoline · O(1) frames"]
    F["(loop 10000)"] --> G["Iter #1<br/>current_exp = (loop 9999)"]
    G --> H["Iter #2<br/>current_exp = (loop 9998)"]
    H --> I["Iter #N<br/>current_exp = (loop 0)"]
    I --> J["✅ Return value"]
  end
  style TCO fill:#f8d7da,stroke:#dc3545
  style Trampoline fill:#d4edda,stroke:#28a745
  style A fill:#f8d7da
  style B fill:#f8d7da
  style C fill:#f8d7da
  style D fill:#f8d7da
...
$ cargo test
running 19 tests
test tests::test_tco_loop_10000 ... ok
...
...
```mermaid
graph LR
  Symbol["Symbol name"] --> Intern["intern()"]
  Intern --> ID["u64 ID"]
  Example1["+"] --> ID123["ID: 0x3F2A"]
  Example2["define"] --> ID456["ID: 0x8B1D"]
  subgraph Benefits["Benefits"]
    Speed["Compare: O(1) int vs O(n) string"]
    Memory["Memory: 1 alloc per unique name"]
    Hash["Hash: FX hasher ~3 CPU instr"]
  end
  style Symbol fill:#e1f5fe,stroke:#4fc3f7
  style Intern fill:#fff3cd,stroke:#ffc107
  style ID fill:#d4edda,stroke:#28a745
  style Example1 fill:#f8f9fa,stroke:#ced4da
  style Example2 fill:#f8f9fa,stroke:#ced4da
  style Benefits fill:#f3e5f5,stroke:#ab47bc
```
...
💡 Think of it this way — `RwLock`: many readers, one writer, never both at once. Like a whiteboard — everyone can read it, but only one person writes at a time.
...
graph LR
  subgraph Old["Traditional: copy"]
    O1["Source"] --> O2["String copy"]
    O2 --> O3["Token vec<br/>heap alloc"]
  end
  subgraph New["This interpreter: zero-copy"]
    N1["Source"] --> N2["&str ref"]
    ...
    while let Some((i, ch)) = chars.next() {
        match ch {
            ...
            _ => {
                ...
                let end = chars.peek().map_or(input.len()-1, |(j,_)| *j-1);
...
```rust
// src/env.rs — at file top, before LispEnv definition, add FX hasher code
...
```mermaid
graph TD
  subgraph Layer["Layers"]
    L1["Lexer<br/>lexer.rs ⟹ Vec&lt;&str&gt;"] --> L2
    L2["Parser<br/>parser.rs ⟹ LispExp AST"] --> L3["Environment<br/>env.rs ⟹ lexical scope"]
    L2 --> L3
    L3 --> L4["Eval<br/>interpreter.rs ⟹ Value"]
    subgraph Shared["Shared Infra"]
      Interner["interner.rs<br/>string interner"]
    end
  end
  L2 -.->|reference| Interner
  L4 -.->|symbol ID| Interner
  style Layer fill:#f8f9fa,stroke:#495057
...
graph TD
  ...
  style Tail fill:#d4edda,stroke:#28a745,stroke-width:3px
  style Ret fill:#cce5ff,stroke:#0d6efd
  style Err fill:#f8d7da,stroke:#dc3545
```
...
if *sym_id == predefined().let_sym {
    ...
    let (names, vals): (Vec<LispExp>, Vec<LispExp>) = /* extract from bindings */;
...
if *sym_id == predefined().and_sym {
    ...
    for arg in &args[..args.len() - 1] {
        ...
        if matches!(v, LispExp::Bool(false) | LispExp::Nil) {
            *env = current_env;
            return Ok(val); // short-circuit, return actual falsy value!
        }
    }
...
Implementation: wrap each binding as a `let` expression, from inside to outside.
...
**Implementation**—Rc shared environment pattern:

1. Create shared Rc environment, initialize all variables to nil
2. With the shared environment as outer, evaluate all lambdas (lambdas see each other through outer)
3. set! real values back into the shared environment
4. Evaluate body in the shared environment
...
**Test**: `(member 2 (list 1 2 3))` → `(2 3)`, not found → `#f`
...
Add `rest: Option<u64>` field to `LispLambda`. Detect `.` symbol as separator when parsing lambda parameters. When calling:
...
`'` is a standalone token in the lexer. In parser, when `'` is detected, expand to `(quote expr)`:
...
fn eval_input(input: &str, env: &mut LispEnv) -> Result<String, String> {
    ...
    while !remaining.is_empty() {
        let (exp, rest) = parse(remaining).map_err(|e| match e {
            lisp_rs_clone::LispErr::Reason(msg) => msg,
        })?;
        remaining = rest;
        match eval(&exp, env) {
            Ok(val) => results.push(val),
            Err(e) => match e {
                lisp_rs_clone::LispErr::Reason(msg) => return Err(msg),
            },
        }
    }
        ...
        if input.starts_with(':') {
            match input.as_str() {
                ...
                ":help" => {
                    ...
                    println!("Built-in functions: + - * / = > < >= <= not list cons car cdr");
...
":info" => {
    ...
    println!("Lists: list cons car cdr append length reverse member");
...
```mermaid
graph TD
  Header["Our Lisp Interpreter"] --> Lexer["Lexer<br/>src/lexer.rs<br/>FSM · Zero-Copy"]
  Header --> Parser["Parser<br/>src/parser.rs<br/>Recursive Descent"]
  Header --> Eval["Eval<br/>src/interpreter.rs<br/>Trampoline · TCO"]
  Header --> Env["Environment<br/>src/env.rs<br/>HashMap · outer chain"]
  Header --> Print["Print<br/>Display impl<br/>Format output"]
  Lexer -.->|Source → Token| Interner["String Interner<br/>src/interner.rs<br/>String ↔ u64"]
  Eval -.-> Interner
  Env -.-> Interner
  style Header fill:#f8f9fa,stroke:#495057,stroke-width:4
  style Lexer fill:#cce5ff,stroke:#0d6efd
  style Parser fill:#d4edda,stroke:#28a745
  style Eval fill:#fff3cd,stroke:#ffc107
  style Env fill:#f8d7da,stroke:#dc3545
  style Print fill:#f0f0f0,stroke:#888
  style Interner fill:#e8daef,stroke:#8e44ad
```

**Key numbers:**
- **~XXX lines of Rust** (zero external dependencies)
- **YY built-in functions** (arithmetic, comparison, list ops, predicates, higher-order)
- **8 special forms** (if, define, lambda, begin, set!, let, cond, and, or, let*, letrec)
...
- **Eval** (125 tests) — all special forms, built-in functions, closures, TCO
...
Steps 52-74: Built-in Functions + REPL
  52-57: <, <=, >=, not, list, cons
  58-62: car, cdr, append, length, reverse, member
  63-66: Type predicates + eq?/equal?
  67-69: map, apply, filter
  70: Variadic lambda      71: quote abbreviation    72: error
  73: Display trait        74: main.rs REPL
```
...
> **Verification**: `cargo test` (171 tests), `cargo run` (interactive REPL)
...
| Comparison | `=` `>` `<` `>=` `<=` |
| Logical | `not` |
| Lists | `list` `cons` `car` `cdr` `append` `length` `reverse` `member` |
